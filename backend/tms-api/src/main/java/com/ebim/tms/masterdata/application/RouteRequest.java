 ...
 *
 * <p>{@code destinationIds} is the whole ordered stop list, not a delta: its array order <em>is</em>
 * the sequence, assigned server-side starting at 1 - see
 * {@code com.ebim.tms.masterdata.domain.Route#replaceStops}. There is no client-supplied
 * sequence number, so a request can never ask for a gap or a duplicate position. {@code
 * RouteService} separately rejects a repeated destination id, which bean validation alone
 * cannot express.
 */
public record RouteRequest(
        ...
        @NotEmpty(message = "a route needs at least one destination stop")
                List<@NotNull UUID> destinationIds) {
}

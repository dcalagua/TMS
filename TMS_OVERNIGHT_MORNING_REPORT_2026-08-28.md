...
environment. The commits are waiting on your local `dev` branch for you to review.
...
4. **`docs/security/STATIC_GUARDS.md`** - every guard the build now enforces, old and new, and what
   each refuses.
5. `git log origin/dev..HEAD --oneline` - everything this chain added, newest first.
